...
class PoliticianExtractor:
    ...
    def __save_politician(politician):
        ...
        PoliticianExtractor.__save_politicans(politicians)
    ...
    def __fetch_politician(self, account_name):
        ...
        return PoliticianExtractor.__build_politician(user)     
    ...
    def fetch_and_save_politican(self, account_name):
        if PoliticianExtractor.__check_if_politician_exists(account_name):
            print(f"Politician with account name {account_name} already exists.")
            return
        
        politician = self.__fetch_politician(account_name)
        PoliticianExtractor.__save_politician(politician)