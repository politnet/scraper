...
def add_twitter_account(scraper, account_name):
    PoliticianUtils(scraper).fetch_and_add_politican(account_name)
...
def scrape_tweets(scraper, account_name):
    if account_name is None:
        TweetExtractor(scraper).get_all_politicians_tweets()
    else:
        TweetExtractor(scraper).get_politician_tweets(account_name)
...
PoliticianUtils.save_politician(        {
            "user_id": "2858025183",
            "user_full_name": "Janusz Korwin-Mikke",
            "user_account_name": "JkmMikke",
            "description": None,
            "last_modified": "2023-10-28T09:02:25.365455+00:00"
        })
